...
run_step "03" "Python workflow and packaged Windows ADS contracts" \
  env PYTHONDONTWRITEBYTECODE=1 python3 -m unittest \
  scripts.tests.test_diagram_workflow \
  scripts.tests.test_packaged_ads_runtime \
  scripts.tests.test_windows_twincat_ads_acceptance \
  scripts.tests.test_windows_ads_candidate_manifest \
  scripts.tests.test_fetch_windows_ads_candidate \
  -v
