"""At-rest validation for Phase 8 runtime-anomaly report artifacts."""
...
def validate_report_files(
    ...
    *,
    ...
    try:
        ...
        return [f"runtime-anomaly report files cannot be read: {exc}"]
    if not isinstance(payload, dict) or not isinstance(schema, dict):
        return ["runtime-anomaly report JSON and schema must be objects"]
    ...
    if not allow_external_test_outputs and isinstance(outputs, Mapping):
        try:
            ...
            failures.append("runtime-anomaly report outputs escape the workspace")
        else:
    ...
    try:
        state = build_live_runtime_anomaly_state(root, timestamp=timestamp)
    except (OSError, ValueError, json.JSONDecodeError) as exc:
        failures.append(f"live runtime-anomaly analysis failed: {exc}")
    else:
        failures.extend(validate_report_payload(payload, expected_state=state))
        if payload.get("input_paths") != list(state.input_paths):
            failures.append("input_paths do not match the complete live runtime-anomaly closure")
        if payload.get("input_digest") != state.input_digest:
            failures.append("input_digest does not match current runtime-anomaly inputs")
