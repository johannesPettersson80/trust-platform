"""CLI orchestration for the report-only combined Phase 5 suite audit."""
...
from .phase5_audit_report import (
    ...
    Phase5AuditProvenance,
    Phase5AuditReport,
    write_reports,
)
from .phase5_audit_validation import validate_report_files, validate_report_payload, validate_schema_contract
...
def default_command(json_path: Path, markdown_path: Path, timestamp: str) -> tuple[str, ...]:
    ...
    return (
        "python3", "scripts/report_phase5_suite_audit.py", "--json-out", json_path.as_posix(),
        "--markdown-out", markdown_path.as_posix(), "--timestamp", timestamp,
    )


def generate_report(root: Path, *, json_path: Path, markdown_path: Path, timestamp: str | None) -> Phase5AuditReport:
    root = root.resolve()
    state = build_live_phase5_state(root, timestamp=timestamp, require_clean_commit=True)
    ...
    failures = validate_report_payload(payload, expected_state=state)
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        report = generate_report(
            args.root, json_path=args.json_out, markdown_path=args.markdown_out, timestamp=args.timestamp
        )
        ...
        failures = validate_report_files(root, json_file, markdown_file, root / REPORT_SCHEMA_PATH)
    except (OSError, ValueError, json.JSONDecodeError) as exc:
        print(f"Phase 5 suite audit failed: {exc}", file=sys.stderr)
        return 2
    if failures:
        for failure in failures:
            print(f"Phase 5 suite audit failed at rest: {failure}", file=sys.stderr)
        return 2
    summary = report.to_dict()["summary"]
    print(
        "Phase 5 suite audit reported: "
        f"{summary['inventory_records']} inventory records, {summary['suite_records']} suites, "
        f"{summary['canonical_areas']} areas, {summary['taxonomy_routes']} routes; report-only exit 0"
    )
    return 0
