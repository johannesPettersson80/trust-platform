"""At-rest validation for Phase 6 requirement/oracle audit artifacts."""
...
from .requirement_oracle_contract import (
    validate_markdown_binding,
    validate_report_payload,
    validate_schema_contract,
)
from .requirement_oracle_live import (
...
def validate_report_files(
    ...
    *,
    ...
    """Recompute live state and bind JSON, Markdown, schema, paths, and source."""
    ...
    try:
        ...
        return [f"requirement/oracle audit files cannot be read: {exc}"]
    if not isinstance(payload, dict) or not isinstance(schema, dict):
        return ["requirement/oracle audit JSON and schema must be objects"]
    ...
    if not allow_external_test_outputs and isinstance(outputs, Mapping):
        try:
            ...
            failures.append("requirement/oracle report outputs escape the workspace")
        else:
    ...
    try:
        state = build_live_requirement_oracle_state(root, timestamp=timestamp)
    except (OSError, ValueError, json.JSONDecodeError) as exc:
        failures.append(f"live requirement/oracle audit failed: {exc}")
    else:
        ...
        if payload.get("input_paths") != list(state.input_paths):
            failures.append("input_paths do not match the complete live requirement/oracle closure")
        if payload.get("input_digest") != state.input_digest:
            failures.append("input_digest does not match current requirement/oracle inputs")
