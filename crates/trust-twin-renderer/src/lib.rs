...
mod wasm {
    ...
    pub async fn init(canvas: HtmlCanvasElement) -> Result<RendererHandle, JsValue> {
        ...
        let surface =
            scena::PlatformSurface::browser_webgpu_canvas_element(canvas.clone(), width, height);
        let renderer = match scena::Renderer::from_surface_async(surface).await {
            Ok(renderer) => renderer,
            Err(_) => {
                let surface =
                    scena::PlatformSurface::browser_webgl2_canvas_element(canvas, width, height);
                scena::Renderer::from_surface_async(surface)
                    .await
                    .map_err(js_error)?
            }
        };
    ...
    pub fn render_frame(handle: &mut RendererHandle) -> Result<(), JsValue> {
        ...
        handle.renderer.set_background(scena::Background::Studio);
        handle
            .renderer
