...
fn non_event_row(document: &Document) -> Result<Vec<DomainRow>, PersistenceError> {
    match document {
        Document::Loss(loss) => Ok(vec![DomainRow::Loss(LossRow {
            ...
            source_path: loss.provenance.source.path.join("/"),
            source_hierarchy: loss.provenance.source.hierarchy.join("/"),
            ...
            time_unsynced: loss.provenance.flags.time_unsynced,
            synthetic_record: loss.provenance.flags.synthetic_record,
            partial_payload: loss.provenance.flags.partial_payload,
        ...
        Document::Placeholder(value) => Ok(vec![DomainRow::Unresolved(UnresolvedRow {
            ...
            source_path: value.provenance.source.path.join("/"),
            source_hierarchy: value.provenance.source.hierarchy.join("/"),
            ...
            time_unsynced: value.provenance.flags.time_unsynced,
            synthetic_record: value.provenance.flags.synthetic_record,
            partial_payload: value.provenance.flags.partial_payload,
