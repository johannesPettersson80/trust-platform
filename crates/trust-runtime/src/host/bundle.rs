...
const PROJECT_FOLDER_NOT_FOUND: &str =
...
pub fn detect_bundle_path(bundle: Option<PathBuf>) -> Result<PathBuf, RuntimeError> {
    ...
    Err(RuntimeError::InvalidBundle(PROJECT_FOLDER_NOT_FOUND.into()))
}

/// Return whether bundle detection failed only because no project was present.
#[must_use]
pub fn is_bundle_not_found(error: &RuntimeError) -> bool {
    matches!(
        error,
        RuntimeError::InvalidBundle(message) if message.as_str() == PROJECT_FOLDER_NOT_FOUND
    )
}
