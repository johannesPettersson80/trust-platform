...
- [x] `OTRM-NFR-006A` Prove production persistence code and every shipped example use native platform path handling, reject Unix-only path literals, and compile the complete database feature set for the Windows target.
  Evidence: the example contract first failed on `unix:///tmp/trust-openot-multi-program.sock`, then passed after every database and multi-program configuration moved to portable TCP control endpoints. The exact broad suite then exposed the required-auth integration assertion (`runtime.control.auth_token required for tcp endpoint`); the same focused test passed after every portable endpoint received the documented local-only example token. `RUSTFLAGS=-Dwarnings cargo check --target x86_64-pc-windows-gnu -p trust-runtime --features openot-database-all` first failed because `open-ot-carriage` was Unix-scoped, then passed after the portable carriage dependency moved to the common dependency set while `open-ot-shm` remained Unix-only and enabled persistence continued to fail closed on non-Unix hosts.
