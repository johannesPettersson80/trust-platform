...
Last verified: 2026-05-01 during the BOARD-08 `runtime/vm/register_ir/tests.rs` split.
...
- [ ] `ARCHPROG-BOARD-08` Runtime large-file split: `runtime-large-file-split-execution-checklist.md`. Status: in progress; Phase 0 and Phase 1 owner/split inventory complete, `RTLARGE-P2-001` through `RTLARGE-P2-003` complete, next top-risk split is `RTLARGE-P2-004`.
...
- [ ] `ARCHIDX-NEXT-05` Continue the active BOARD-08 top-risk split work in `runtime-large-file-split-execution-checklist.md`; next open detail row is `RTLARGE-P2-004` unless a smaller behavior-lock split is needed first.
