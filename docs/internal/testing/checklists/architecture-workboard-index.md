...
- [ ] `ARCHPROG-BOARD-08` Runtime large-file split: `runtime-large-file-split-execution-checklist.md`. Status: in progress; Phase 0 through Phase 3 complete, remaining hotspot rows track optional follow-on splits, and the next risk-ranked hotspot is `RTLARGE-HOT-12`.
...
- [ ] `ARCHIDX-NEXT-08` Continue active BOARD-08 work in `runtime-large-file-split-execution-checklist.md`; next risk-ranked hotspot is `RTLARGE-HOT-12` (`crates/trust-runtime/src/runtime/vm/register_ir/tier1.rs`).
