...
- [ ] `ARCHPROG-BOARD-08` Runtime large-file split: `runtime-large-file-split-execution-checklist.md`. Status: in progress; Phase 0 through Phase 3 complete, remaining hotspot rows track optional follow-on splits, and the next risk-ranked hotspot is `RTLARGE-HOT-07`.
...
- [ ] `ARCHIDX-NEXT-05` Continue the active BOARD-08 work in `runtime-large-file-split-execution-checklist.md`; next risk-ranked hotspot is `RTLARGE-HOT-07` (`crates/trust-runtime/src/bin/trust-dev/agent.rs`) unless a smaller behavior-lock split is needed first.
