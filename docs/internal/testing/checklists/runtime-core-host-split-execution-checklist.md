...
- [ ] `RTSPLIT-P6-002` Move core cycle execution only after I/O boundary behavior locks pass. Progress: cycle and I/O boundary locks passed, and the portable `ReadyTask` plus deterministic ready-task ordering helper moved to `trust-runtime-core`; the full `execute_cycle` loop remains host-owned until the process-image and I/O-driver port boundary is split without moving driver implementations.
...
- [ ] `RTSPLIT-P6-005` Move retain snapshot policy/model into core. Progress: `RetainPolicy` and `RestartMode` moved to `trust-runtime-core`; `RetainSnapshot` remains host-owned until a compatibility plan preserves or replaces the public `RetainSnapshot::from_runtime(&Runtime)` constructor without silently changing the API.
