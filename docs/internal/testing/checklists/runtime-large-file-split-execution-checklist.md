...
Status: In progress; Phase 0 through Phase 3 complete; remaining hotspot rows track optional follow-on splits, and the next risk-ranked hotspot is `RTLARGE-HOT-09` (`crates/trust-runtime/src/runtime/vm/register_ir/lower.rs`)
...
- [x] `RTLARGE-RULE-02` Every existing Rust source file over 1,000 lines must have owner, responsibility statement, and split plan or waiver. Evidence: current `FULLMAP-CHECK-10` pass lists all 3 remaining runtime hotspots over 1,000 lines with owner and split plan after `runtime/vm/call.rs`, `web/config_ui_routes.rs`, `runtime/vm/register_ir/tests.rs`, `memory.rs`, `bytecode_vm_core.rs`, `agent_command.rs`, `runtime/vm/register_ir.rs`, and `trust-dev/agent.rs` were split below the threshold.
...
- [x] `RTLARGE-RULE-04` Files over 2,500 lines are release-blocking for unrelated growth until split or waiver. Evidence: `FULLMAP-CHECK-10` passed on 2026-05-01 and reports the largest remaining runtime file is `crates/trust-runtime/src/runtime/vm/register_ir/lower.rs` at 1,303 lines after the Phase 2, `RTLARGE-HOT-06`, `RTLARGE-HOT-04`, `RTLARGE-HOT-05`, and `RTLARGE-HOT-07` splits, so no >2,500-line runtime file remains.
...
- [x] `RTLARGE-EXIT-01` Every >1,000-line runtime file has owner/split note. Evidence: `FULLMAP-CHECK-10` passed on 2026-05-01 with all 3 remaining runtime hotspots listed after the `call.rs`, `config_ui_routes.rs`, `register_ir/tests.rs`, `memory.rs`, `bytecode_vm_core.rs`, `agent_command.rs`, `register_ir.rs`, and `trust-dev/agent.rs` splits.
