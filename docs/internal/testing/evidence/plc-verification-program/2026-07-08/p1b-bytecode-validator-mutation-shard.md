...
Date: 2026-07-09; refreshed 2026-07-14 for the duplicate-section product fix
...
- `cargo-mutants 27.0.0` package discovery returned zero candidates for the
  ...
  commit `8437e60842680bf07236fbd40f5ba23926463520`.
...
- SHA-256:
  `6e2c0eeb9fd016823bc6c62561bf37680a1b84fdb303e3e3dec8077dafe44e38`

The latest 2026-07-14 refresh binds the measured pilot to the clean
duplicate-standardized-section metadata checkpoint. The case-table digest
changed because `VM_SEAM_VALID_001` now maps that decoder behavior; the
validator source, associated blocked case IDs, commands, and outcomes remain
unchanged. The report was regenerated by the runner rather than hand-edited.
...
- The source checkpoint includes the separately reviewed bounded STRING/WSTRING
  product fix. This mutation artifact remains adequacy evidence only: it changes
  no CI enforcement, skill, agent instruction, or spec-gap status and creates no
  product-proof claim.
...
```text
cd "$HOME/projects/trust-platform-bytecode-count-502fff48"
export TMPDIR="$HOME/.cache/codex-targets/trust-platform-gate/tmp"
python3 scripts/bytecode_validator_mutation.py \
  --target-dir "$HOME/.cache/codex-targets/trust-platform-gate" \
  --output-json /tmp/8437-bytecode-validator-mutation.json \
  --output-markdown /tmp/8437-bytecode-validator-mutation.md
```
