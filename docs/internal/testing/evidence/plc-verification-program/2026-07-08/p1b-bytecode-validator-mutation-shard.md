...
- `cargo-mutants 27.0.0` package discovery returned zero candidates for the
  ...
  commit `744d4df631b1ac88da20cd62703c8ff882e76113`.
...
The latest 2026-07-13 refresh binds the measured pilot to the clean
specification-source audit implementation commit. The metadata validator input
closure changed, so the mechanically regenerated `VM_SEAM_VALID_001` file
digest changed. The case IDs, mutant selectors, commands, and outcomes are
...
python3 scripts/bytecode_validator_mutation.py \
  ...
  --output-json /tmp/p1a-bytecode-validator-mutation.json \
  --output-markdown /tmp/p1a-bytecode-validator-mutation.md
```
