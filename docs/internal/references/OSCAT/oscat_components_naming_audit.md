...
Status: Complete for the OSCAT Components release.

Scope:

- `examples/oscat_components_*`
...
```bash
find examples -maxdepth 1 -type d -name 'oscat_components_*' | sort | wc -l
find examples/tutorials -type f \( -name '*.st' -o -name '*.ST' \) | sort | wc -l
rg -n "\b[a-z][A-Za-z0-9]*_[A-Za-z0-9_]*\b" examples/oscat_components_* examples/tutorials -g '*.st' -S
rg -n "\b[a-z][A-Za-z0-9]*_[A-Za-z0-9_]*\b" examples/plant_demo examples/filling_line -g '*.st' -S
```

Observed results:

- 40 OSCAT Components comparison projects exist.
- 15 tutorial ST files were checked.
- No snake_case ST identifiers remain in the new OSCAT Components examples.
...
- [x] OSCAT Components examples checked.
