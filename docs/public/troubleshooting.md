...
| Windows simulator says `runtime.control.auth_token required for tcp endpoint` | retry **Start** with the current extension; it upgrades an older writable local project before launch and replaces known placeholders such as `some-secret-value` with a random token. If the file is read-only or the endpoint is remote, the exact action-required error remains visible | [runtime.toml](reference/config/runtime-toml.md) |
| ADS discovery finds nothing | select **Discover ADS devices** once; it searches this computer and the local network. If the device is running but automatic identity is blocked, open **Advanced** and add its bare Host/IP and optional AMS Net ID | [Beckhoff ADS](connect/external-systems/ads.md) |
| an ADS device is found but a port is unavailable | expand **Technical details** and check which logical services responded. `851`–`854`, `301`, and `501` are checked automatically; add a non-standard logical service under **Advanced** | [Beckhoff ADS](connect/external-systems/ads.md) |
| same-computer Windows ADS asks for a route | do not add a self-route. Confirm the local ADS router and intended ADS runtime are running; truST uses the installed native Windows ADS client for this path | [Beckhoff ADS](connect/external-systems/ads.md) |
| the Run card says Running but Devices says Stopped | wait for **Starting** to settle, then use **Stop** and **Start** once. All simulator surfaces should follow the same accepted session; collect the truST output if they remain different | [Debugging And Runtime Panel](operate/debugging-and-runtime-panel.md) |
