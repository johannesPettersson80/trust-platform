# OSCAT BASIC Smoke Example

This example is the reference consumer for `libraries/oscat_basic`.

Use it when you want to see how a normal truST project should consume the
currently shipped OSCAT BASIC slice through `[dependencies]`.
...
- `trust-lsp.toml`: project settings and dependency on `libraries/oscat_basic`
...
This keeps the example aligned with the larger date/time slice now shipped in
...
slice, the full current generator/memory/logic-others slices (`A_TRIG`,
...
```bash
cargo run -p trust-runtime --bin trust-runtime -- test --project crates/trust-runtime/tests/fixtures/oscat_basic/core
```
